...
public class BeanPropertyValidator<T> implements INullAcceptingValidator<T> {
    ...
    private transient final Logger log = LoggerFactory.getLogger(BeanPropertyValidator.class);
     ...
     */
    public BeanPropertyValidator(final Class<?> beanClass, String propertyName, final Class<?>... groups) {
    ...
    public void validate(IValidatable<T> validatable) {
        ...
        Set<?> violations = validator.validateValue(beanClass, propertyName, value, groups);

        // For each violations we set the component state to ERROR
        for (Object v : violations) {
            ConstraintViolation<?> violation = (ConstraintViolation<?>) v;
            validatable.error(new ValidationError().setMessage(propertyName + " " + violation.getMessage()));
            log.error("Violation = " + propertyName + " " + violation.getMessage() + " - value was " + value);
        }
